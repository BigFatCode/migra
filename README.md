...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create migra-6d23
  fly apps create migra-6d23-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app migra-6d23
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app migra-6d23-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'migra-6d23-staging' does not match app name in config file 'migra-6d23'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name migra-6d23-db
  fly postgres attach --postgres-app migra-6d23-db --app migra-6d23

  fly postgres create --name migra-6d23-staging-db
  fly postgres attach --postgres-app migra-6d23-staging-db --app migra-6d23-staging
  ```
